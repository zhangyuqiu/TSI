...
void APP_DRIVER_STATE_FSM_Tasks ( void )
{
    ...
    switch ( app_driver_state_fsmData.state )
    {
        ...
        case DRIVE_SETUP: 
        {
            ...
            if(saftyloop == 0){
                app_driver_state_fsmData.state = IDLE;
            } else if(DriverButton_Pushed & ~throttleImplausibility & throttlegreaterthan & brakePressed & MC_Activate){
        ...
        case DRIVE:
        {
            ...
            if(saftyloop == 0){
                app_driver_state_fsmData.state = IDLE;
            } else if(throttleImplausibility | (throttlepressed & brakePressed) | !MC_Activate){
                app_driver_state_fsmData.state = DRIVE_SETUP;
            } else if(overCurr){
...
//    MC_Activate = mc_active();
    ...
    boardtemp = get_ADCCh(9)/0x10;
    CoolTemp_1 = get_NCD9830_reading(2);
    CoolTemp_2 = get_NCD9830_reading(1);
    FlowRate = get_NCD9830_reading(0);
//    throttle = get_ADCCh(14)/0x10;
    
    throttlegreaterthan = throttle > 0x26;
    throttlepressed = throttle > 0x50;
...
//    can_send_bytes(0x300,conditions,0,0,0,0,0,0,0);
//    can_send_bytes(0x300,PC_Ready,throttleImplausibility,overCurr,saftyloop,DriverButton_Pushed,brakePressed,MC_Activate,throttlegreaterthan);
